...
def train():
    ...
    dataset = LoRADataset("data/training/lora/lora_qa_sft.jsonl", tokenizer, max_length=1024)
    ...
    cpt_path = Path("models/checkpoints/lora/lora_adapter_cpt.pt")
    if cpt_path.exists():
        print(f"Loading existing CPT LoRA weights from {cpt_path} before SFT...")
        model.load_state_dict(torch.load(cpt_path)["model_state_dict"], strict=False)
    ...
    epochs = 8  # A bit higher to ensure SFT formatting sticks with small dataset
    batch_size = 2
    ...
    chkpt_path = out_dir / "lora_adapter_sft.pt"
    checkpoint = {
    ...
    print(f"LoRA SFT adapter saved to {chkpt_path}")
    ...
    isro_prompt = "### Question:\nWhat is remote sensing?\n\n### Answer:\n"
    dpdpa_prompt = "### Question:\nWho is a Data Principal under the DPDPA?\n\n### Answer:\n"
    
    print("\nISRO Prompt test:")
    print(generate_text(fresh_model, tokenizer, isro_prompt, 40, device))
    
    print("\nDPDPA Prompt test:")
    print(generate_text(fresh_model, tokenizer, dpdpa_prompt, 40, device))
    
    print("\nALL SFT CRITERIA PASSED.")
